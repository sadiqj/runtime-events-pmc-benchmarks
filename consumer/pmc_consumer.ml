(*
   ...
   Supports both built-in runtime phase spans (GC events) and custom
   user-defined span events (application-level profiling).
...
(* Unboxed array access for perf_sample arrays — local_ to accept stack-allocated arrays *)
...
(* Key: (domain_id, event_name) for user spans *)
...
let runtime_begin domain_id ts phase (local_ samples : perf_sample array) =
  if array_length samples > 0 then
...
let runtime_end domain_id ts phase (local_ samples : perf_sample array) =
  if array_length samples > 0 then begin
    ...
    | Some begin_state ->
      ...
      emit_span_end (runtime_phase_name phase) domain_id ts begin_state samples
  end
  ...
  if array_length samples > 0 then begin
    ...
    | End ->
      ...
      | Some begin_state ->
        ...
        emit_span_end event_name domain_id ts begin_state samples
  end
